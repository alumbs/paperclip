...
function parseWindsurfToolCallEvent(event: Record<string, unknown>, ts: string): TranscriptEntry[] {
...
export function parseWindsurfStdoutLine(line: string, ts: string): TranscriptEntry[] {
  const normalized = normalizeWindsurfStreamLine(line);
  ...
  if (type === "system") {
    ...
    if (subtype === "init") {
      const sessionId =
        ...
        asString(parsed.sessionID);
      return [{ kind: "init", ts, model: asString(parsed.model, "windsurf"), sessionId }];
    }
  ...
  if (type === "tool_call") {
    return parseWindsurfToolCallEvent(parsed, ts);
  }
