...
export function parseWindsurfJsonl(stdout: string) {
  ...
  for (const rawLine of stdout.split(/\r?\n/)) {
    const line = normalizeWindsurfStreamLine(rawLine).line;
    if (!line) continue;
...
export function isWindsurfUnknownSessionError(stdout: string, stderr: string): boolean {
  ...
  return /unknown\s+(session|flow)|session\s+.*\s+not\s+found|flow\s+.*\s+not\s+found|resume\s+.*\s+not\s+found|could\s+not\s+resume/i.test(
    haystack,
  );
}
