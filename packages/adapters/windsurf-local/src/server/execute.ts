...
import { parseWindsurfJsonl, isWindsurfUnknownSessionError } from "./parse.js";
import { normalizeWindsurfStreamLine } from "../shared/stream.js";
...
function resolveWindsurfBillingType(env: Record<string, string>): "api" | "subscription" {
  return hasNonEmptyEnvValue(env, "WINDSURF_API_KEY") || hasNonEmptyEnvValue(env, "OPENAI_API_KEY")
    ? "api"
    : "subscription";
}

function resolveWindsurfBiller(
  billingType: "api" | "subscription",
  provider: string | null,
): string {
  if (billingType === "subscription") return "windsurf";
...
function resolveProviderFromModel(model: string): string | null {
  ...
  if (trimmed.includes("sonnet") || trimmed.includes("claude") || trimmed.includes("opus") || trimmed.includes("haiku")) return "anthropic";
  if (trimmed.startsWith("gpt") || trimmed.startsWith("o")) return "openai";
  if (trimmed.startsWith("gemini")) return "google";
  if (trimmed.startsWith("cascade")) return "windsurf";
  return null;
}
...
function windsurfSkillsHome(): string {
  return path.join(os.homedir(), ".windsurf", "skills");
}

async function buildWindsurfSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-windsurf-skills-"));
...
type EnsureWindsurfSkillsInjectedOptions = {
...
export async function ensureWindsurfSkillsInjected(
  onLog: AdapterExecutionContext["onLog"],
  options: EnsureWindsurfSkillsInjectedOptions = {},
) {
  ...
  const skillsHome = options.skillsHome ?? windsurfSkillsHome();
  try {
    await fs.mkdir(skillsHome, { recursive: true });
  } catch (err) {
    await onLog(
      "stderr",
      `[paperclip] Failed to prepare Windsurf skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const windsurfSkillEntries = await readPaperclipRuntimeSkillEntries(config, __moduleDir);
  const desiredWindsurfSkillNames = resolvePaperclipDesiredSkillNames(config, windsurfSkillEntries);
  if (!executionTargetIsRemote) {
    await ensureWindsurfSkillsInjected(onLog, {
      skillsEntries: windsurfSkillEntries.filter((entry) => desiredWindsurfSkillNames.includes(entry.key)),
    });
  }
  ...
  const billingType = resolveWindsurfBillingType(effectiveEnv);
  ...
  if (executionTargetIsRemote) {
    try {
      localSkillsDir = await buildWindsurfSkillsDir(config);
      await onLog(
        "stdout",
        `[paperclip] Syncing workspace and Windsurf runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
      ...
      if (remoteHomeDir && preparedExecutionTargetRuntime.assetDirs.skills) {
        const remoteSkillsDir = path.posix.join(remoteHomeDir, ".windsurf", "skills");
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[paperclip] Windsurf session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[paperclip] Windsurf session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const commandNotes = (() => {
    const notes: string[] = [];
    if (autoTrustEnabled) {
      notes.push("Auto-added --trust to bypass interactive prompts.");
    }
    notes.push("Prompt is piped to Windsurf via stdin.");
  ...
  const buildArgs = (resumeSessionId: string | null) => {
    const args = ["flow", "-p", "--output-format", "stream-json", "--workspace", effectiveExecutionCwd];
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    const emitNormalizedStdoutLine = async (rawLine: string) => {
      const normalized = normalizeWindsurfStreamLine(rawLine);
    ...
    return {
      proc,
      parsed: parseWindsurfJsonl(proc.stdout),
    };
  };
  ...
  const toResult = (
    attempt: {
      ...
      parsed: ReturnType<typeof parseWindsurfJsonl>;
    },
    ...
    const fallbackErrorMessage =
      parsedError ||
      stderrLine ||
      `Windsurf exited with code ${attempt.proc.exitCode ?? -1}`;
    ...
    return {
      ...
      biller: resolveWindsurfBiller(billingType, providerFromModel),
      model,
      billingType,
  ...
  try {
    ...
    if (
      ...
      isWindsurfUnknownSessionError(initial.proc.stdout, initial.proc.stderr)
    ) {
      await onLog(
        "stdout",
        `[paperclip] Windsurf resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
