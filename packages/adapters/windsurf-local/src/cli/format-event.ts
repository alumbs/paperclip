...
export function printWindsurfStreamEvent(raw: string, _debug: boolean): void {
  const line = normalizeWindsurfStreamLine(raw).line;
  if (!line) return;
  ...
  if (type === "system") {
    ...
    if (subtype === "init") {
      ...
      const details = [sessionId ? `session: ${sessionId}` : "", model ? `model: ${model}` : ""]
        .filter(Boolean)
        .join(", ");
      console.log(pc.blue(`Windsurf init${details ? ` (${details})` : ""}`));
      return;
    }
